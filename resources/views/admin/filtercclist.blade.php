...
foreach($data as $k=>$v) $$k=$v; $stato_pratica   =  config('constants.stato_pratica');
                ...
                <div class="box-body"  id="table_filtered_content">
                    <table id="example1" class="table table-bordered table-striped">
                        ...
                        @foreach ($ccList as $airlines)
                            <?php
                            $data_partenza=date('Y-m-d', $airlines->depdate);
                            $data_apertura=date('Y-m-d', $airlines->sigdate);

                            $invio_quietanza="";
                            if ($airlines->date_conferma_quietanza > 0)   $invio_quietanza=date('Y-m-d',
                                    $airlines->date_conferma_quietanza);

                            $data_pagamento="";
                            if ($airlines->payed > 0)   $data_pagamento=date('Y-m-d', $airlines->closuredate);
