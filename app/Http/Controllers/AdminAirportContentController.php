...
/*
    ...
    /*
     * name:    editairportcontent
     * params:  $acid
     * return:
     * desc:    Change the details of the Airport Content admin
     */
    public function  editairportcontent($acid)  {
        $inputData      = Input::all();
        ...
        if(!empty($inputData) && count($inputData)> 0 && isset($inputData['addac'])) {
            ...
            if ($validator->fails()) {
                ...
                $status  = AdminAirportContent::updateAirportContent($acdata, $acid);
                //return json_encode($status);
                Session::flash('statmsg', $status['msg']);
                Session::flash('status', $status['stat']);
            }
        }
        $acontentdetails    = AdminAirportContent::getAirportContentList(array('acId'=>$acid));
        if($acontentdetails)
            $data['acDetails']    = $acontentdetails[0];
        else $data['acDetails']   = (object) array();
        $data['airportsList']  = AdminSBAirports::getAirportsList();
        return \View::make('admin.airportcontentadd')->with('data', $data);
    }
